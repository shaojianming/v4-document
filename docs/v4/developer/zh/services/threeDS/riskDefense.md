---
title: riskDefense
date: 2023-05-06 11:52:59
permalink: /pages/d96acf/
---

# Integration Guide for 3DS riskDefense Component

## Initialization

### load JS

```
https://pay-cdn.pingpongx.com/production-fra/static/riskDefense/pingpongRiskDefense.min.js
```



###  initialize JS

```js
const pp = new PingPongRiskDefense({
    env: 'SANDBOX'|'PRODUCTION',
    accId: string,
    clientId: string,
    requestId: string,
    merchantUserId: string,
    merchantTransactionId: string
})
```

| Attribute             | Type   | Required | Description                                                                                                                                         |
|-----------------------|--------|----------|-----------------------------------------------------------------------------------------------------------------------------------------------------|
| env                   | string | M        | The current calling service environment：<br />SANDBOX<br />PRODUCTION                                                                               |
| accId                 | string | M        | PingPong merchant store ID                                                                                                                          |
| requestId             | string | C        | When conducting server-to-server integration, transmit the parameter                                                                                |
| merchantUserId        | string | C        | Member ID, the user's member ID on the merchant website. Applicable to quick payment scenarios. This field can be left blank if the user is a guest |
| merchantTransactionId | string | C        | Cashier should transmit `merchantTransactionId`                                                                                                     |



## Obtain 3DS and risk control parameters

###  Obtain generatedData

Before initiating a payment request to PingPongCheckout, perform this step to obtain 'generatedData' and pass it to the paymentapiduring payment.

```js
const generatedData = pp.getGeneratedData()
```



The generatedData structure returned is as follows:

```js
{
    browserInfo: {
        acceptHeader: string,
        colorDepth: number,
        language: string,
        screenHeight: number
        screenWidth: number
        timeZoneOffset: number
        userAgent: string
        javaEnabled: boolean
        javaScriptEnabled: boolean
    },
    jsGeneratedData: {
        fingerPrint: string,
        forterSiteId: string,
        forterTokenCookie：string | null
    }
}    
```



## Unified order message assembly

###  Fill for browserInfo

Request PingPongChekout <a href="/pages/a2c224/"> Accept a Payment </a>. Transferring the obtained 'generatedData' in the parameter 'browserInfo'.

| Attribute                | Type   | Description                                                                                                                                                                              |
|-------------------|--------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| language          | String | The value of the "navigator.language" in the shopper's browser, as defined in IETF BCP 47                                                                                                |
| screenWidth       | int    | The screen width of the cardholder's terminal, measured in pixels                                                                                                                        |
| javaEnabled       | bool   | The ability of the cardholder's terminal to execute Java                                                                                                                                 |
| javaScriptEnabled | bool   | The indication of whether the shopper's browser can execute JavaScript,If the field doesn't exist, it is assumed to have a default value of true                                         |
| acceptHeader      | String | An example of HTTP response header information can be as follows：text/html, application/xhtml+xml, application/xml;q=0.9, image/webp, image/apng, *;q=0.8                                |
| colorDepth        | String |                                                                                                                                                                                          |
| screenHeight      | int    | The screen height of the cardholder's terminal, measured in pixels                                                                                                                       |
| jetLag            | int    | The time difference between UTC time and the cardholder's browser local time, measured in minutes                                                                                        |
| userAgent         | String | An example of browser user agent information can be as follows：Mozilla/5.0 (Macintosh; Intel Mac OS X 10_14_0) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/72.0.3626.121 Safari/537.36 |

###  Fill for jsGeneratedData

| Attribute                | Type     | Description   |
|-------------------|--------|------|
| fingerPrint       | string | Device Fingerprint |
| forterSiteId      | string | Website Identification |
| forterTokenCookie | string |      |



### Requesting Payment

After completing the additional parameter filling mentioned above, you may also need to fill in<a href="/pages/a2c224/"> Accept a Payment </a>the necessary business parameters for the interface,Upon successfully initiating the payment, the response message will carry `threeDUnionParams` parameter

Request Example：

```json
{
    "clientId":"2090020315464510103001",
    "accId":"2090020315464510103001",
    "amount":3,
    "version":"1.0",
    "currency":"USD",
    "merchantTransactionId":"{{merchantTransID}}",
    "paymentType":"SALE",
    "shopperResultUrl":"https://aa.com",
    "shopperCancelUrl":"https://aa.com",
    "signType":"MD5",
    "threeDSecure":"N",
    "sign":"{{sign}}",
    "requestId":"TEST12345-122452",
    "acquirerType":"PAY",
    "notificationUrl":"https://test-acquirerpay.pingpongx.com/qa/notify",
    "tradeCountry":"TH",
    "merchantSource":"shopLine",
    "browserInfo":{
        "userAgent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/70.0.3538.110 Safari/537.36",
        "acceptHeader":"text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8",
        "language":"nl-NL",
        "colorDepth":24,
        "screenHeight":723,
        "screenWidth":1536,
        "timeZoneOffset":0,
        "javaEnabled":true
    },
    "payMethodInfo":{
        "card":{
            "cvv":"103",
            "expireMonth":"12",
            "expireYear":"2024",
            "holderFirstName":"James",
            "holderLastName":"LeBron",
            "number":"5200000000001096"
        }
    },
    "billing":{
        "city":"Birmingham",
        "country":"US",
        "email":"3ds@pingpongx.com",
        "firstName":"James",
        "lastName":"LeBron",
        "phone":"18301770495",
        "postcode":"35222",
        "state":"AL",
        "street":"1986 Broad Street"
    },
    "shipping":{
        "city":"Miami",
        "country":"US",
        "email":"t_email",
        "firstName":"Jamesbb",
        "lastName":"LeBronbb",
        "phone":"3055787342",
        "postcode":"33131",
        "state":"FL",
        "street":"701 Brickell Avenue, Suite 2700",
        "lastModifierStreetTime":"20191225162010",
        "lastModifierPhoneTime":"20191225162010"
    },
    "customer":{
        "customerId":"20191201001",
        "email":"3ds@pingpongx.com",
        "firstName":"James",
        "identificationId":"",
        "identificationType":"ID",
        "lastName":"LeBron",
        "loginIp":"222.126.52.24",
        "loginTime":"201912010032",
        "orderCountry":"US",
        "orderIp":"222.126.52.23",
        "orderTime":"20191201001",
        "payCountry":"US",
        "payIp":"222.126.52.25",
        "phone":"18301770495",
        "preferentialOrder":"Y",
        "registerCountry":"US",
        "registerIp":"222.126.52.26",
        "registerRange":"3",
        "registerTerminal":"PC",
        "registerTime":"20191201122000"
    }
}


```



Response Example：

```json
{
    "accId": "2090020315464510103001",
    "amount": "3",
    "clientId": "2090020315464510103",
    "currency": "USD",
    "language": "en",
    "merchantTransactionId": "cj-test20230328141733",
    "notificationUrl": "https://test-acquirerpay.pingpongx.com/qa/notify",
    "paymentType": "DEBIT",
    "relateTransactionId": "PS23032814173445637",
    "shopperResultUrl": "https://aa.com",
    "sign": "934817101344F5803B437086B886D7EE",
    "signType": "MD5",
    "status": "PROCESSING",
    "threeDContinue": "true",
    "threeDUnionParams": {
        "threeDRedirectUrl": "https://dev-acquirer-static.pingpongx.com/payment/threeDs.html?code=",
        "threeDResult": "{\"ACSUrl\":\"https://centinelapistag.cardinalcommerce.com/V2/Cruise/StepUp\",\"authAccessToken\":\"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9._.k71qXGmm-M3uj5oCZzw_4Sr9vASgFFe9szsoO9GqRcI\",\"authenticationTransactionId\":\"ZSlZU5YUN3NRGHOEnhj0\",\"cardEnrolled\":\"Y\",\"channelName\":\"CyberSource\",\"threeDRequestId\":\"cj-test20230328141733\",\"version\":\"1.0.2\"}"
    },
    "transactionId": "PS23032814173445637",
    "transactionTime": "1679984255000"
}
```





##  3DS Required

### threeDContinue

-  When `threeDUnionParams.threeDContinue` is set to `true`，it need to trigger 3D verification.
-  When `threeDUnionParams.threeDContinue` is set to `false`,it will be redirected to the merchant's payment result page `shopperResultUrl`.



## 3DS Challenge Page

###  To trigger the 3D Secure challenge page using the JS "trigger" method

To initiate the 3D Secure challenge page, you can use the JS "trigger" function by passing the `threeDUnionParams` returned from the payment API. The risk management JS will guide the browser to the 3D challenge page where the cardholder needs to complete the 3D verification successfully in order to proceed with the transaction.

```js
pp.trigger(threeDUnionParams)
```

### To trigger the 3D Secure challenge page using PingPongCheckout's provided intermediate page

To trigger the 3D Secure challenge page using PingPongCheckout's provided intermediate page, you can extract the threeDUnionParams.threeDRedirectUrl parameter from the response message of the <a href="/pages/a2c224/"> Accept a Payment </a> API. You can then perform a GET request to this URL, and subsequently, PingPongCheckout's intermediate page will automatically redirect to the 3D Secure challenge page.



